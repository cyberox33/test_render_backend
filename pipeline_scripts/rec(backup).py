...
from pptx.enum.shapes import MSO_SHAPE # For manual title check
...
try:
    ...
    print(f"Warning: Could not load config ({e}). Falling back to env vars.")
...
def fetch_fragment_info(supabase_client: Client, fragment_ids: List[str]) -> Dict[str, Dict[str, Any]]:
    ...
    try:
        ...
        response = supabase_client.table("fragment_info").select(
            "fragment, description, category, frag_names" # Added frag_names
        ).in_("fragment", unique_fragment_ids).execute()
        if response.data: return {f['fragment']: f for f in response.data}
        else: print(f"Warning: No fragment info found matching fragments: {unique_fragment_ids}."); return {frag_id: {"fragment": frag_id, "description": "N/A", "category": "N/A", "frag_names": f"Fragment {frag_id}"} for frag_id in unique_fragment_ids}
    except Exception as e: print(f"Error fetching fragment info: {e}"); traceback.print_exc(); return {frag_id: {"fragment": frag_id, "description": "N/A", "category": "N/A", "frag_names": f"Fragment {frag_id}"} for frag_id in fragment_ids}
...
def group_data_by_fragment(followup_responses: List[Dict[str, Any]], questions_details: Dict[int, Dict[str, Any]]) -> Dict[str, Dict[str, Any]]:
    ...
    grouped_data = defaultdict(lambda: {"fragment_info": None, "questions_data": []}); fragment_ids_found = set()
    for resp in followup_responses:
        ...
        fragment_ids_found.add(cleaned_fragment_id)
    ...
    if fragment_ids_found:
        ...
        for frag_id, data in grouped_data.items(): data["fragment_info"] = all_fragment_info.get(frag_id, {"fragment": frag_id, "description": "N/A", "category": "N/A", "frag_names": f"Fragment {frag_id}"})
...
def format_chunk_for_prompt(fragment_info: Dict[str, Any], questions_chunk: List[Dict[str, Any]]) -> str:
    ...
    output.append(f"Fragment Name: {frag_display_name}")
    output.append(f"Fragment Description: {fragment_info.get('description', 'N/A')}")
    output.append(f"(Internal Fragment ID: {fragment_info.get('fragment', 'N/A')})")
...
def generate_synthesize_roadmaps_prompt(partial_roadmaps_list: List[Dict[str, Any]], fragment_id: str, fragment_name: str) -> str:
    ...
    Includes paragraph formatting and summarization instructions.
    """
...
def add_custom_roadmap_details_slide(
    prs: Presentation,
    title_text: str,
    details: Dict[str, Any], # Pass the consolidated_details dictionary
    font_size: int = 8,
    # Dimensions from user input
    left_col_dims: tuple = (0.07, 0.25, 7.07, 7.14), # L, T, W, H in inches
    right_col_dims: tuple = (7.25, 0.25, 5.68, 7.14) # L, T, W, H in inches
):
    """
    Adds a slide using a Blank layout and creates two custom text boxes
    with specified dimensions and formatting for roadmap details.
    """
    try:
        blank_layout = prs.slide_layouts[DEFAULT_LAYOUT_BLANK]
        slide = prs.slides.add_slide(blank_layout)
    except IndexError:
        print(f"Error: Layout index {DEFAULT_LAYOUT_BLANK} (Blank) not found. Using layout 0 as fallback.")
        try: slide = prs.slides.add_slide(prs.slide_layouts[0])
        except IndexError: print("Error: Cannot add slide, no suitable layouts found."); return

    # Manually Add Title Text Box
    try:
        title_left, title_top, title_width, title_height = Inches(0.5), Inches(0.1), Inches(12), Inches(0.5)
        title_box = slide.shapes.add_textbox(title_left, title_top, title_width, title_height)
        title_tf = title_box.text_frame; title_tf.text = title_text
        p = title_tf.paragraphs[0]; p.font.bold = True; p.font.size = Pt(18); p.alignment = PP_ALIGN.LEFT
        title_tf.word_wrap = False; title_tf.auto_size = MSO_AUTO_SIZE.SHAPE_TO_FIT_TEXT
    except Exception as e_title: print(f"Error adding manual title box: {e_title}")

    # Prepare Content for Columns
    methodology_content = details.get("how_methodology", "N/A")
    tools_content = details.get("tools_platforms_used", "N/A")
    subtasks_content = details.get("estimate_per_subtask", "N/A")
    right_column_text = f"Tools/Platforms (Overall):\n{tools_content}\n\nSubtask Estimates (Aggregated):\n{subtasks_content}"

    # Add Left Text Box (Methodology)
    try:
        l, t, w, h = [Inches(d) for d in left_col_dims]
        left_box = slide.shapes.add_textbox(l, t, w, h)
        tf_left = left_box.text_frame; tf_left.clear(); tf_left.word_wrap = True
        tf_left.auto_size = MSO_AUTO_SIZE.NONE # Disable auto-fit
        tf_left.vertical_anchor = MSO_ANCHOR.TOP
        # Add content and format paragraph by paragraph
        paragraphs_left = str(methodology_content).split('\n')
        for para_text in paragraphs_left:
             p_left = tf_left.add_paragraph()
             p_left.alignment = PP_ALIGN.LEFT # Or JUSTIFY
             run_left = p_left.add_run(); run_left.text = para_text; run_left.font.size = Pt(font_size)
    except Exception as e_left: print(f"Error adding/formatting left column: {e_left}"); traceback.print_exc()

    # Add Right Text Box (Tools/Subtasks)
    try:
        l, t, w, h = [Inches(d) for d in right_col_dims]
        right_box = slide.shapes.add_textbox(l, t, w, h)
        tf_right = right_box.text_frame; tf_right.clear(); tf_right.word_wrap = True
        tf_right.auto_size = MSO_AUTO_SIZE.NONE # Disable auto-fit
        tf_right.vertical_anchor = MSO_ANCHOR.TOP
        # Add content and format paragraph by paragraph
        paragraphs_right = str(right_column_text).split('\n')
        for para_text in paragraphs_right:
             p_right = tf_right.add_paragraph()
             p_right.alignment = PP_ALIGN.LEFT # Or JUSTIFY
             run_right = p_right.add_run(); run_right.text = para_text; run_right.font.size = Pt(font_size)
    except Exception as e_right: print(f"Error adding/formatting right column: {e_right}"); traceback.print_exc()
...
def build_ppt_report(session_id: str, report_data: Dict[str, Dict[str, Any]], template_path_relative: Optional[str] = "template.pptx") -> Optional[io.BytesIO]:
    ...
    # Slide 1: Title Slide (using layout 1)
    try:
        ...
        else: print("Warning: Title slide layout missing title.")
        if subtitle_shape: add_text_to_shape(subtitle_shape, f"Session ID: {session_id}\nGenerated: {time.strftime('%Y-%m-%d')}")
        else: print("Warning: Title slide layout missing subtitle.")
    ...
    for fragment_id in sorted_fragment_ids:
        ...
        # Slides: Roadmap Overview & CUSTOM Details
        ...
        if is_valid_parsed_roadmap or has_graph_data:
            ...
            try:
                ...
                # Use consolidated description for overview content
        ...
        graph_title = f"{frag_display_name}: Roadmap Phase Durations"
        if graph_buffer:
            try:
                graph_layout = None
            ...
            if synthesis_roadmaps_prompt:
                 ...
                 if final_roadmap_txt:
                     ...
                     if parsed_final_roadmap:
                         if parsed_final_roadmap.get('fragment_name') != frag_display_name: print(f"Updating fragment name in parsed data to '{frag_display_name}'"); parsed_final_roadmap['fragment_name'] = frag_display_name
